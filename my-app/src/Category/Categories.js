...
function Categories() {
    ...
    const onSubmit = (data, e) => {
        ...
        axios.post('http://localhost:5000/CategoryData', data)
            .then((data) => {
                console.log(data);
    ...
    function EditformSubmit() {
        ...
        axios.put('http://localhost:5000/CategoryData', formData).then((data) => { console.log(data) }).catch((err) => { console.log(err) });
        closeModal();
        alert("Refresh page to see changes")
    }
    function deletemember(_id) {
        console.log(_id)
        axios.delete('http://localhost:5000/CategoryData', { data: { _id: _id } })
                ...
                <div class="col-lg-10 col-md-12 mt-5 pt-5">
                    ...
                    <Modalcomp title="Add a Category">
                        <form onSubmit={handleSubmit(onSubmit)}>
                            ...
                            {errors.CategoryName && <p className="text-muted ">Category Name is required.</p>}
                            <br />
                    ...
                    {edit && <Modal
                        ...
                        <TextField label="Category Name" onChange={e => setFormData({ ...formData, CategoryName: e.target.value })} defaultValue={edit.CategoryName}></TextField><br />
                        <br />
                        <label>Status -- </label>
                    ...
                    <div className="mt-5 ">
                        {category && <Tables Edit={(data) => { setedit(data); setmodalIsOpen(true); setFormData({ CategoryName: data.CategoryName, Status: data.Status, ImageURL: data.ImageURL, Keywords: data.Keywords, _id: data._id }) }} Delete={(_id) => { deletemember(_id); console.log(_id) }} rows={category}
                            columns={['No', 'Category Name', 'Image URL', 'Status', 'Keywords', 'Created Date (yyyy-mm-dd)', 'Actions']} />}
                    </div>
                </div>
            </div>
